...
def get_adni_datasets(adni_project_dir: str, train_ratio: float = 0.8):
    """
    Load ADNI dataset with subject-level splitting to prevent data leakage across longitudinal scans.
    """
    labels_csv = os.path.join(adni_project_dir, 'Metadata', 'ADNI_labels.csv')
    inventory_csv = os.path.join(adni_project_dir, 'Metadata', 'ADNI_MRI_inventory.csv')

    if not os.path.exists(labels_csv) or not os.path.exists(inventory_csv):
        raise FileNotFoundError(f"ADNI metadata CSVs not found in {os.path.join(adni_project_dir, 'Metadata')}")
...
def train_adni_mri(epochs: int = 5, batch_size: int = 32, architecture: str = 'ensemble'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device} | Cohort: ADNI | Architecture: {architecture}")

    train_dataset, val_dataset, label_map = get_adni_datasets(PROJECT_DIR)
    ...
    output_path = os.path.join(output_dir, 'adni_alzheimer_model.pth')

    best_acc = 0.0

    print("\n--- Starting ADNI Model Retraining ---")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train ADNI MRI Diagnostic Model')
    ...
    train_adni_mri(epochs=args.epochs, batch_size=args.batch_size, architecture=args.architecture)
