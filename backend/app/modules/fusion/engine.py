...
class MultimodalFusion:
    def predict(
        ...
        active_weights = {k: DEFAULT_WEIGHTS[k] for k in modalities}
        ...
        adj_total = sum(norm_weights[k] * (0.5 + 0.5 * modalities[k]['confidence']) for k in modalities)
        ...
        fused_conf = round((fused_conf_raw / max(adj_total, 0.01)) * agreement_bonus * 100, 1)
        fused_conf = max(10.0, min(fused_conf, 99.0))

        contributions = {k: round(norm_weights[k] * 100, 1) for k in modalities}
