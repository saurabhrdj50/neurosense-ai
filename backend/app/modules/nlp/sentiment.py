...
class SentimentAnalyzer:
    """
    Research-grade Clinical NLP Module.
    Supports ClinicalBERT, BioBERT, and DeBERTa-v3 transformers for medical sentiment and cognitive risk analysis.
    """
    def __init__(self, model_name: str = "emilyalsentzer/Bio_ClinicalBERT") -> None:
        ...
        if HAS_TRANSFORMERS:
            try:
                ...
                logger.info("ClinicalBERT transformer '%s' loaded successfully.", model_name)
            except Exception as e:
                logger.warning("ClinicalBERT transformer unavailable (%s), trying TextBlob fallback.", e)

        try:
            from textblob import TextBlob
            self._TextBlob = TextBlob
            self._nlp_available = True
            logger.info("TextBlob loaded fallback.")
        except ImportError:
