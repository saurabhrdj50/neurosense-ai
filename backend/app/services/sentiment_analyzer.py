...
Analyzes patient speech / text for emotional and cognitive patterns
associated with Alzheimer's disease progression.

Uses TextBlob for lightweight sentiment + keyword-based cognitive
marker detection. Falls back gracefully if transformers unavailable.
"""
...
class SentimentAnalyzer:
    """
    Analyzes patient text for:
    - Overall sentiment polarity (positive / negative / neutral)
    - Dominant emotion detection
    - Cognitive decline linguistic markers
    - Alzheimer's risk indicators from speech patterns
    """

    def __init__(self) -> None:
        self._nlp_available = False
        self._TextBlob: Optional[type] = None
        try:
    ...
    def analyze(self, text: str) -> Dict[str, Any]:
        ...
        markers = self._detect_cognitive_markers(text)
        linguistics = self._linguistic_features(text)
        risk = self._cognitive_risk(markers, linguistics)
        ...
        return {
            ...
            'cognitive_markers_found': markers['found'],
            'cognitive_marker_count': markers['count'],
    ...
    def _get_sentiment(self, text: str) -> Dict[str, Any]:
        ...
        if self._nlp_available and self._TextBlob is not None:
            ...
            polarity = float(blob.sentiment.polarity)  # type: ignore[union-attr]
        else:
    ...
    def _rule_sentiment(self, text: str) -> float:
        """Simple positive/negative word counting fallback."""
    ...
    def _detect_emotions(self, text: str) -> Dict[str, Any]:
        ...
        if scores:
            try:
                dominant = max(scores.keys(), key=lambda k: scores[k])  # type: ignore[arg-type, misc]
    ...
    def _detect_cognitive_markers(self, text: str) -> Dict[str, Any]:
        lower = text.lower()
        ...
        return {'found': found, 'count': sum(len(v) for v in found.values())}
    ...
    def _linguistic_features(self, text: str) -> Dict[str, Any]:
        ...
        clause_markers = [',', ' and ', ' but ', ' or ', ' because ', ' although ',
                          ' while ', ' when ', ' if ', ' that ']
    ...
    def _cognitive_risk(self, markers: Dict[str, Any], linguistics: Dict[str, Any]) -> Dict[str, Any]:
        score = 0

        score += markers['count'] * 8
    ...
    def _generate_insights(
        ...
        if sentiment['label'] == 'Negative':
            insights.append(
                "Negative emotional tone detected — patient may be experiencing distress."
            )
        ...
        found_markers = markers.get('found', {})
        if 'memory_loss' in found_markers:
            insights.append("Memory-related language patterns identified.")
        if 'word_finding' in found_markers:
            insights.append("Word-finding difficulty indicators present.")
        if 'disorientation' in found_markers:
