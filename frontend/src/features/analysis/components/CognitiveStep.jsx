...
export function CognitiveStep({ cognData, setCognData }) {
  return (
    <div className="space-y-5">
      ...
      <p style={{ fontSize: 13, color: '#9CA3AF' }}>Mini‑Mental State Examination (MMSE) style questions. Rate each 0–10.</p>
      
      <div className="space-y-4">
        {COG_QUESTIONS.map(q => (
          <div key={q.id} className="p-4 rounded-xl" style={{ background: '#1F2937', border: '1px solid #374151' }}>
            <div className="flex items-start justify-between mb-3 gap-3">
              <div>
                <p style={{ fontSize: 14, fontWeight: 600, color: '#FFFFFF' }}>{q.label}</p>
                <p style={{ fontSize: 12, color: '#9CA3AF', marginTop: 2 }}>{q.question}</p>
              </div>
              <input type="number" min={0} max={q.type === 'scale' ? 10 : 5}
                value={cognData[q.id] ?? ''}
                onChange={e => setCognData(d => ({ ...d, [q.id]: Number(e.target.value) }))}
                className="w-20 px-3 py-2 rounded-xl text-sm text-center flex-shrink-0" 
                placeholder="0-10" 
                style={{ background: '#111827', border: '1px solid #374151', color: '#FFFFFF' }} />
            </div>
            {q.type === 'scale' && (
      ...
      <InfoBox type="tip">
        Higher scores indicate better cognitive function. Scores below 24/30 may indicate cognitive impairment.
      </InfoBox>
    </div>
  )
}
