      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {RISK_FIELDS.map(f => (
          <motion.button key={f.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            ...
            style={{ background: risk[f.id] ? 'rgba(99,102,241,0.12)' : 'rgba(255,255,255,0.03)', border: risk[f.id] ? '1px solid rgba(99,102,241,0.4)' : '1px solid rgba(255,255,255,0.06)', cursor: 'pointer' }}>
            <div className="w-5 h-5 rounded flex items-center justify-center flex-shrink-0"
              style={{ background: risk[f.id] ? 'linear-gradient(135deg, #6366f1, #a855f7)' : 'rgba(255,255,255,0.06)', border: risk[f.id] ? 'none' : '1px solid rgba(255,255,255,0.12)' }}>
              {risk[f.id] && <Check size={12} color="white" />}
            </div>
            <span style={{ fontSize: 13, color: risk[f.id] ? '#a5b4fc' : '#94a3b8', fontWeight: risk[f.id] ? 500 : 400 }}>{f.label}</span>
          </motion.button>
        ))}
      </div>
      <div className="mt-2">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: '#e2e8f0' }}>Genomic Data (optional)</span>
        </div>
        <DropZone accept={{ 'text/plain': ['.txt'], 'application/octet-stream': [] }} label="Upload DNA File"
          hint="Raw DNA text file — APOE ε4 & TREM2 analysis" file={dnaFile} onFile={setDnaFile} onClear={() => setDnaFile(null)} type="default" />
      </div>
    </div>
  )
}
